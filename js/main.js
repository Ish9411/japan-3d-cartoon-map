/* Cartoon 3D Japan Route Planner
 * - Stylized island shapes (not geographic)
 * - Click or pick cities to draw curved routes
 * - Exaggerated landmarks for popular attractions
 * - Built for local use with VS Code Live Server (or any static server)
*/
...
(function() {
  ...
  scene.background = new THREE.Color(0x7ab6e6); // sky-ish
  ...
  // ---- Create islands via simple outline shapes (cartoon blobs) ----
  function roundedPolygonShape(points, radius) {
    // Simple rounded path for a series of points in xz-plane
    const shape = new THREE.Shape();
    if (points.length < 3) {
      console.warn('Need 3+ points');
      return shape;
    }
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      // line to 'to' of current segment
      shape.lineTo(to.x, to.y);

      // round the corner into the next segment using a quadratic curve
      const mid = pNext.clone();
  ...
  // Approximate island outlines (very stylized)
  // Coordinates roughly in a plane (x, z)
  const hokkaidoPts = [
    [300, -60], [350, -20], [320, 20], [280, 40], [240, 20], [260, -30]
  ];
  const honshuPts = [
    [-260, -70], [-200, -50], [-120, -40], [0, -20], [100, -10], [170, 10],
    [200, 40], [160, 80], [80, 90], [10, 80], [-70, 70], [-130, 40], [-200, 0], [-260, -30]
  ];
  const shikokuPts = [
    [-120, -120], [-60, -110], [0, -115], [-10, -150], [-90, -155], [-140, -140]
  ];
  const kyushuPts = [
    [-260, -160], [-210, -150], [-160, -150], [-140, -190], [-200, -210], [-260, -195]
  ];
  const okinawaPts = [
    [-320, -260], [-300, -255], [-280, -270], [-300, -285]
  ];
  ...
  // Slight vertical offsets so shapes don't z-fight
  ...
  // ---- City & attraction database (positions in global coordinates) ----
  // Positions are eyeballed relative to island outlines above
  const places = [
    ...
    // Attractions
    { name: "Mount Fuji", kind:"attraction", position: new THREE.Vector3(80, 10, 10), note:"Japan's highest peak." },
    { name: "Tokyo Tower", kind:"attraction", position: new THREE.Vector3(125, 10, 18), note:"Iconic red tower." },
    { name: "Fushimi Inari Torii", kind:"attraction", position: new THREE.Vector3(-44, 9, 24), note:"Thousand torii gates." },
    { name: "Osaka Castle", kind:"attraction", position: new THREE.Vector3(-28, 9, 35), note:"Historic castle." },
    { name: "Itsukushima Torii", kind:"attraction", position: new THREE.Vector3(-170, 9, 36), note:"Floating shrine gate." },
    { name: "Nara Park", kind:"attraction", position: new THREE.Vector3(-17, 9, 58), note:"Friendly deer & temples." },
  ];
  ...
  // ---- Attractions: exaggerated, cartoon builds ----
  ...
  function addFuji(position) {
    ...
    const cone = new THREE.Mesh(base, rockMat);
    cone.position.copy(position).add(new THREE.Vector3(0, 12, 0));
    cone.castShadow = true;

    const top = new THREE.Mesh(snow, snowMat);
    top.position.copy(position).add(new THREE.Vector3(0, 30, 0));
    top.castShadow = true;

    attractionGroup.add(cone, top);

    const label = makeLabel("Mount Fuji");
    label.position.copy(position).add(new THREE.Vector3(0, 44, 0));
    scene.add(label);
  }
  ...
  function addTokyoTower(position) {
    ...
    parts.add(m1, m2, m3);
    parts.position.copy(position);
    parts.castShadow = true;
    attractionGroup.add(parts);
  }

  function addTorii(position) {
    const mat = makeToonMaterial(0xff3d00);
    const group = new THREE.Group();
    const pillarGeo = new THREE.CylinderGeometry(1.2, 1.2, 10, 8);
    const barGeo = new THREE.BoxGeometry(10, 1.2, 2);
    ...
    bar.position.set(0, 10.5, 0);

    group.add(p1, p2, bar);
    group.position.copy(position);
    group.castShadow = true;
    attractionGroup.add(group);
  }

  function addOsakaCastle(position) {
    const baseMat = makeToonMaterial(0x6b4f39);
    const roofMat = makeToonMaterial(0x2ba84a);
    const group = new THREE.Group();
    ...
    function level(w, h, y) {
      ...
      const roof = new THREE.ConeGeometry(w * 0.75, h * 0.8, 4);
      const r = new THREE.Mesh(roof, roofMat);
      r.position.y = y + h * 0.9;
  ...
  function addNaraDeer(position) {
    // Simple stylized "deer" from boxes
    const mat = makeToonMaterial(0x9c6b3d);
    const group = new THREE.Group();

    const body = new THREE.Mesh(new THREE.BoxGeometry(8, 4, 3), mat);
    body.position.y = 6;
    ...
    const antler = new THREE.Mesh(new THREE.BoxGeometry(0.6, 2.4, 0.6), new THREE.MeshToonMaterial({ color: 0xead39c }));
    antler.position.set(6, 10, 0);

    group.add(body, head, antler);
    group.position.copy(position);
    attractionGroup.add(group);
  }
  ...
  for (const p of places) {
    if (p.kind === "city") cityPins.push(addCityMarker(p));
  }
  ...
  for (const p of places) {
    ...
    else if (p.name.includes("Torii") && p.name.includes("Fushimi")) addTorii(pos);
  ...
  // ---- Routing (curved tube between cities) ----
  ...
  function drawRoute(aName, bName) {
    ...
    // Curve arc elevated in the middle
    const mid = start.clone().lerp(end, 0.5);
    const up = new THREE.Vector3(0, 40 + start.distanceTo(end) * 0.05, 0);
    mid.add(up);

    const curve = new THREE.QuadraticBezierCurve3(start, mid, end);
    const tube = new THREE.TubeGeometry(curve, 64, 1.3, 12, false);
    const mat = new THREE.MeshToonMaterial({ color: 0xff6b57 });
    const mesh = new THREE.Mesh(tube, mat);
    mesh.castShadow = true;
    routesGroup.add(mesh);

    // Add a moving "train" sphere along the route
    ...
    // Display distance (rough straight-line)
    const km = (start.distanceTo(end) * 6).toFixed(0); // scale factor (not to scale)
    const out = document.getElementById('distance');
    out.textContent = `Approx. straight-line distance: ~${km} km (stylized scale)`;
  }
  ...
  // Clicking pins to set "from" then "to"
  ...
  function onClick(e) {
    ...
    const intersects = raycaster.intersectObjects(cityPins, false);
    if (intersects.length) {
      const place = intersects[0].object.userData.place;
      if (!awaitingSecond) {
        fromSel.value = place.name;
        awaitingSecond = true;
      } else {
        toSel.value = place.name;
        awaitingSecond = false;
        drawRoute(fromSel.value, toSel.value);
      }
    }
  }
