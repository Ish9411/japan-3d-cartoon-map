// ES-module build for r160+ — works on GitHub Pages (no bundler)
...
(function() {
  ...
  function makeToonMaterial(colorHex) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      data[i*3+0] = shade; data[i*3+1] = shade; data[i*3+2] = shade;
    }
    const gradientMap = new THREE.DataTexture(data, size, 1, THREE.RGBFormat);
    gradientMap.needsUpdate = true;
    return new THREE.MeshToonMaterial({ color: new THREE.Color(colorHex), gradientMap });
  }
  ...
  sea.rotation.x = -Math.PI/2; sea.position.y = -5; sea.receiveShadow = true; scene.add(sea);
  ...
  function roundedPolygonShape(points, radius) {
    ...
    for (let i=0;i<points.length;i++){
      const p= v(points[i][0], points[i][1]);
      const n= v(points[(i+1)%points.length][0], points[(i+1)%points.length][1]);
      const dir = n.clone().sub(p).normalize();
      const len = n.distanceTo(p);
      const r = Math.min(radius, len/2);
      const from = p.clone().add(dir.clone().multiplyScalar(len - r));
      const to   = p.clone().add(dir.clone().multiplyScalar(r));
      shape.lineTo(to.x, to.y);
      shape.quadraticCurveTo(p.x, p.y, from.x, from.y);
    }
    shape.closePath();
    return shape;
  }

  function makeIsland(name, pts, opt) {
    const group = new THREE.Group(); group.name = name;
    const geom = new THREE.ExtrudeGeometry(roundedPolygonShape(pts, opt.cornerRadius||6), {
      depth: opt.depth||8, bevelEnabled:true, bevelThickness:3, bevelSize:2, bevelSegments:2, curveSegments:12, steps:1
    });
    const land = new THREE.Mesh(geom, makeToonMaterial(opt.color||0x74c365));
    land.rotation.x = -Math.PI/2; land.castShadow = true; land.receiveShadow = true; group.add(land);
  ...
  islands.Hokkaido.position.y=0.4; islands.Honshu.position.y=0.2; islands.Shikoku.position.y=0.15; islands.Kyushu.position.y=0.12; islands.Okinawa.position.y=0.1;
  ...
  function makeLabel(text){
    ...
    div.textContent=text; div.style.padding="3px 6px"; div.style.background="rgba(0,0,0,0.7)";
    div.style.color="white"; div.style.borderRadius="6px"; div.style.fontSize="11px";
    return new CSS2DObject(div);
  }

  function addCityMarker(place){
    const pin=new THREE.Mesh(cityGeo, cityMaterial);
    pin.position.copy(place.position); pin.position.y+=3; pin.castShadow=true; pin.userData={place};
    markersGroup.add(pin);
    const label=makeLabel(place.name); label.position.set(0,10,0); pin.add(label);
    return pin;
  }

  const attractionGroup=new THREE.Group(); scene.add(attractionGroup);

  function addFuji(pos){
    const base=new THREE.ConeGeometry(22,36,6), snow=new THREE.ConeGeometry(16,10,6);
    const rockMat=makeToonMaterial(0x3a7f4e), snowMat=new THREE.MeshToonMaterial({color:0xffffff});
    const cone=new THREE.Mesh(base,rockMat); cone.position.copy(pos).add(new THREE.Vector3(0,12,0)); cone.castShadow=true;
    const top =new THREE.Mesh(snow,snowMat); top.position.copy(pos).add(new THREE.Vector3(0,30,0)); top.castShadow=true;
    attractionGroup.add(cone,top);
    const label=makeLabel("Mount Fuji"); label.position.copy(pos).add(new THREE.Vector3(0,44,0)); scene.add(label);
  }
  function addTokyoTower(pos){
    const mat=makeToonMaterial(0xff3d00), g=new THREE.Group();
    const m1=new THREE.Mesh(new THREE.CylinderGeometry(1,6,16,6),mat); m1.position.y=8;
    const m2=new THREE.Mesh(new THREE.CylinderGeometry(1,4,20,6),mat); m2.position.y=26;
    const m3=new THREE.Mesh(new THREE.ConeGeometry(3,8,6),mat);        m3.position.y=40;
    g.add(m1,m2,m3); g.position.copy(pos); g.castShadow=true; attractionGroup.add(g);
  }
  function addTorii(pos){
    const mat=makeToonMaterial(0xff3d00), g=new THREE.Group();
    const p1=new THREE.Mesh(new THREE.CylinderGeometry(1.2,1.2,10,8),mat); p1.position.set(-4,5,0);
    const p2=new THREE.Mesh(new THREE.CylinderGeometry(1.2,1.2,10,8),mat); p2.position.set(4,5,0);
    const bar=new THREE.Mesh(new THREE.BoxGeometry(10,1.2,2),mat); bar.position.set(0,10.5,0);
    g.add(p1,p2,bar); g.position.copy(pos); g.castShadow=true; attractionGroup.add(g);
  }
  function addOsakaCastle(pos){
    const baseMat=makeToonMaterial(0x6b4f39), roofMat=makeToonMaterial(0x2ba84a), g=new THREE.Group();
    function level(w,h,y){
      const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,w),baseMat); m.position.y=y; m.castShadow=true; g.add(m);
      const r=new THREE.Mesh(new THREE.ConeGeometry(w*0.75,h*0.8,4),roofMat); r.position.y=y+h*0.9; r.rotation.y=Math.PI/4; r.castShadow=true; g.add(r);
    }
    level(18,6,4); level(14,6,12); level(10,6,20); g.position.copy(pos); attractionGroup.add(g);
  }
  function addNaraDeer(pos){
    const mat=makeToonMaterial(0x9c6b3d), g=new THREE.Group();
    ...
    const leg=new THREE.BoxGeometry(1,6,1);
    for (let x of [-3,3]) for (let z of [-1,1]){ const l=new THREE.Mesh(leg,mat); l.position.set(x,3,z); g.add(l); }
    ...
    g.add(body,head,antler); g.position.copy(pos); attractionGroup.add(g);
  }
  ...
  function drawRoute(aName,bName){
    const a=places.find(p=>p.name===aName), b=places.find(p=>p.name===bName); if(!a||!b) return;
    routesGroup.clear();
    ...
    let t=0; (function run(){ if(!train.parent) return; t+=0.002; if(t>1) t=0; train.position.copy(curve.getPointAt(t)); requestAnimationFrame(run); })();
    document.getElementById('distance').textContent = `Approx. straight-line distance: ~${(start.distanceTo(end)*6).toFixed(0)} km (stylized scale)`;
  }

  const fromSel=document.getElementById('from'), toSel=document.getElementById('to'),
        routeBtn=document.getElementById('routeBtn'), clearBtn=document.getElementById('clearBtn'),
        auto=document.getElementById('autorotate');

  (function populate(){
    const names=places.filter(p=>p.kind==='city').map(p=>p.name);
    for (const n of names){
      const a=document.createElement('option'); a.value=a.textContent=n; fromSel.appendChild(a);
      const b=document.createElement('option'); b.value=b.textContent=n; toSel.appendChild(b);
    }
  ...
  const raycaster=new THREE.Raycaster(), mouse=new THREE.Vector2(); let awaitingSecond=false;
  renderer.domElement.addEventListener('click', (e)=>{
    const r=renderer.domElement.getBoundingClientRect();
    mouse.x=((e.clientX-r.left)/r.width)*2-1; mouse.y=-((e.clientY-r.top)/r.height)*2+1;
    raycaster.setFromCamera(mouse,camera);
    const hit=raycaster.intersectObjects(cityPins,false)[0];
    if(hit){ const name=hit.object.userData.place.name;
